...
export default function Demo() {
  ...
  const badgeStyle = (status) => {
    ...
    if (status === "Approved")
      return { ...base, background: "rgba(34,197,94,.15)", color: "#22c55e", border: "1px solid rgba(34,197,94,.35)" };
    if (status === "Review")
      return { ...base, background: "rgba(245,158,11,.15)", color: "#f59e0b", border: "1px solid rgba(245,158,11,.35)" };
    return { ...base, background: "rgba(239,68,68,.15)", color: "#ef4444", border: "1px solid rgba(239,68,68,.35)" };
  };
    ...
    <div style={{ minHeight: "100vh", background: "#0b0f14", color: "#e5e7eb" }}>
      <div style={{ maxWidth: 980, margin: "0 auto", padding: "40px 20px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
          ...
          <div style={{ border: "1px solid rgba(148,163,184,.18)", borderRadius: 14, padding: 14, background: "rgba(15,23,42,.35)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.1fr .9fr", gap: 18 }}>
          <form onSubmit={runVerification} style={{ border: "1px solid rgba(148,163,184,.18)", borderRadius: 16, padding: 18, background: "rgba(2,6,23,.45)" }}>
            <div style={{ fontWeight: 800, marginBottom: 10 }}>Run a verification</div>

            <label style={labelStyle}>Full name</label>
            <input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="e.g., Jane Customer" style={inputStyle} required />
            ...
            <input type="file" accept="image/*,.pdf" onChange={(e) => setFileName(e.target.files?.[0]?.name || "")} style={{ ...inputStyle, padding: "10px 12px" }} />
            <div style={{ fontSize: 12, color: "#64748b", marginTop: 6 }}>
              Note: the file is not processed in demo mode. The name triggers simulated risk signals for a credible walkthrough.
            </div>

            {err ? (
              <div style={{ marginTop: 12, padding: 12, borderRadius: 12, border: "1px solid rgba(239,68,68,.35)", background: "rgba(239,68,68,.12)", color: "#fecaca" }}>
                {err}
              </div>
            ) : null}
            ...
            <button
              ...
              style={{
                ...
                background: loading ? "rgba(251,191,36,.12)" : "linear-gradient(90deg, rgba(251,191,36,.95), rgba(245,158,11,.95))",
                color: "#0b0f14",
                fontWeight: 900,
          ...
          <div style={{ border: "1px solid rgba(148,163,184,.18)", borderRadius: 16, padding: 18, background: "rgba(2,6,23,.45)" }}>
            ...
            {!result ? (
              <div style={{ color: "#94a3b8", lineHeight: 1.6 }}>
                ...
                <div style={{ marginTop: 12, padding: 12, borderRadius: 12, border: "1px solid rgba(148,163,184,.18)", background: "rgba(15,23,42,.35)" }}>
              ...
              <>
                ...
                <div style={{ marginTop: 14, padding: 14, borderRadius: 14, border: "1px solid rgba(148,163,184,.18)", background: "rgba(15,23,42,.35)" }}>
